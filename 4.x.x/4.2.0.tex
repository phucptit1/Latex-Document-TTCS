    ...
    \begin{longtable}{|
        ...
        Nông dân chỉ xem được thông tin của chính mình sau khi đã đăng nhập. Thông tin hiển thị gồm: tên đăng nhập, họ tên, địa chỉ email, vị trí GPS mặc định và ngày tham gia hệ thống. \\
        \hline
        ...
        \begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
            ...
            \item \textbf{Vị trí GPS mặc định:} kéo thả ghim trên bản đồ để cập nhật tọa độ mặc định.
        \end{itemize}
    ...
    \begin{longtable}{|
        ...
        \begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
            ...
            \item \textbf{Định vị vị trí:} ghim vị trí trực tiếp trên bản đồ để tự động lấy tọa độ GPS chính xác đến 8 chữ số thập phân cho vĩ độ (\texttt{gps\_lat}) và kinh độ (\texttt{gps\_lng}). Hệ thống tự động gọi API định vị ngược để lấy tên tỉnh/thành phố (\texttt{province}).
        ...
        \begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
            ...
            \item Đồng bộ trực tiếp các thông số tọa độ (\texttt{gps\_lat}, \texttt{gps\_lng}) và tỉnh thành (\texttt{province}) của ruộng này sang các cột tương ứng (\texttt{default\_gps\_lat}, \texttt{default\_gps\_lng}, \texttt{default\_province}) trong bảng \texttt{user\_profiles} để đảm bảo tính tương thích ngược với thiết kế code NestJS thực tế.
        \end{enumerate} \\
        \hline
    \end{longtable}
    ...
    \begin{longtable}{|
        ...
        Không bắt buộc. Hệ thống tự động lấy tọa độ GPS mặc định đã được thiết lập trong quá trình Onboarding (QĐ-ND-01.3) hoặc cập nhật gần nhất từ Field Book (QĐ-ND-03.5). Nông dân có thể thay đổi vị trí cho lần chẩn đoán hiện tại bằng 2 cách:
        \begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
            \item \textbf{Chọn từ danh sách ruộng đã lưu:} chọn nhanh một ruộng từ Field Book để lấy tọa độ tương ứng.
            \item \textbf{Chẩn đoán tại vị trí mới:} ghim vị trí mới trực tiếp trên bản đồ hoặc dùng nút định vị GPS của thiết bị.
        \end{itemize}
        \textbf{Quy tắc xử lý:} Nếu không có tọa độ, hệ thống sử dụng dữ liệu thời tiết mặc định để đảm bảo chức năng chẩn đoán vẫn hoạt động bình thường. Nếu có tọa độ, hệ thống gọi API thời tiết để lấy dữ liệu phục vụ bộ điều chỉnh điểm số của mô hình AI. \\
        \hline
    ...
    \begin{longtable}{|
        ...
        \end{itemize}
        \textbf{Quy tắc xử lý:} Danh sách hiển thị phân trang, hỗ trợ tải thêm bài viết theo yêu cầu. Không yêu cầu đăng nhập để xem danh sách. \\
        \hline
        ...
        \begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
            ...
            \item \textbf{Hình ảnh:} không bắt buộc, tối đa 4 ảnh mỗi bài. Định dạng cho phép: JPG, JPEG, PNG, WEBP. Dung lượng tối đa 5MB/ảnh.
            \item \textbf{Thẻ tag:} không bắt buộc, tối đa 5 tags, mỗi tag tối đa 20 ký tự.
        \end{itemize}
        \textbf{Quy tắc xử lý:} Ảnh được tải lên thông qua dịch vụ Cloudinary trước khi lưu bài viết. Bài viết được lưu và hiển thị ngay sau khi đăng thành công. \\
        \hline
        ...
        \textbf{Ràng buộc:} Nội dung sau chỉnh sửa phải đáp ứng đầy đủ các ràng buộc như khi tạo mới (QĐ-ND-07.2). \\
        \hline
        ...
        \begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
            ...
            \item \textbf{Phản hồi (Reply) cấp 2:} nhấn nút ``Phản hồi'' dưới một bình luận để mở ô nhập và gửi phản hồi thụt lề ngay bên dưới bình luận cha.
        \end{enumerate}
        \textbf{Ràng buộc:} Nội dung bình luận tối thiểu 1 ký tự, tối đa 1.000 ký tự. Hệ thống tự động cập nhật số lượng bình luận của bài viết sau mỗi lần thêm thành công. \\
        \hline
        \textbf{QĐ-ND-07.7} & \textbf{Chỉnh sửa / Xóa bình luận} &
        Nông dân chỉ được phép chỉnh sửa hoặc xóa bình luận do chính mình viết. \newline
        \textbf{Ràng buộc:} Nội dung sau chỉnh sửa phải đáp ứng giới hạn độ dài như khi tạo mới. Xóa bình luận cha sẽ đồng thời ẩn toàn bộ các phản hồi con liên quan. \\
        \hline
        \textbf{QĐ-ND-07.8} & \textbf{Vote bài viết \& bình luận} &
        Nông dân phải đăng nhập để thực hiện vote. Hệ thống áp dụng cơ chế Toggle thông minh:
        \begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
        ...
        \end{itemize}
        \textbf{Ràng buộc:} Mỗi nông dân chỉ có duy nhất 1 trạng thái vote trên mỗi bài viết hoặc bình luận tại một thời điểm. Hệ thống cho phép tự vote bài viết của chính mình. \\
        \hline
    \end{longtable}
